...
router.post("/", (req, res) => {
  ...
  let prompt = `Relevant short Keywords for '${job_role}', ordered by most popular, respond a list in comma-separated formating`
  ...
  async function createTextRequest(
    ...
    const results = {
      related: result.choices[0].text.split(",").map((job) => {
        return job.replace(/\n\n/g, "").trimLeft()
      })
    }
    res.send(results)
